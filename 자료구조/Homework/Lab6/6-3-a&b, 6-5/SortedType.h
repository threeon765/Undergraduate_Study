...
template <class ItemType>
void PrintList(SortedType<ItemType>& one) {
    int checker;
    for (int i = 0; i < one.LengthIs(); i++) {
        one.GetNextItem(checker);
        std::cout << checker << " ";
    }
    std::cout << std::endl;
}
...
template <class ItemType>
void MergeLists(SortedType<ItemType>& one, SortedType<ItemType>& another, SortedType<ItemType>& result) {
    int checker;
    for (int i = 0; i < one.LengthIs(); i++) {
        one.GetNextItem(checker);
        result.InsertItem(checker);
    }
    for (int i = 0; i < another.LengthIs(); i++) {
        another.GetNextItem(checker);
        result.InsertItem(checker);
    }
}
...
void SortedType<ItemType>::MergeLists(SortedType<ItemType>& other, SortedType<ItemType>& result) {
    NodeType<ItemType>* node = this->listData;
    NodeType<ItemType>* onode = other.listData;
    while (node != NULL) {
        onode = other.listData;
        bool overlap = false;
        while (onode != NULL) {
            if (node->info == onode->info) {
                overlap = true;
            }
            onode = onode->next;
        }
        if (overlap == false) {
            result.InsertItem(node->info);
        }
        node = node->next;
    }
    onode = other.listData;
    while (onode != NULL) {
        result.InsertItem(onode->info);
        onode = onode->next;
    }
}